...
def sync_lock_acquire(context, engine_id, forced=False):
    ...
    LOG.info('Trying to acquire lock with engine Id: %s' % engine_id)
    lock_status = db_api.sync_lock_acquire(context, engine_id)
    if lock_status:
        return True
    ...
    while retries > 0:
        ...
        LOG.info('Retry lock again with engine Id: %s' % engine_id)
        lock_status = db_api.sync_lock_acquire(context, engine_id)
        if lock_status:
            return True
        retries = retries - 1
    ...
    if forced:
        lock_status = db_api.sync_lock_steal(context, engine_id)
        if not lock_status:
            return False
        else:
            return True
    ...
    LOG.error(_LE('Not able to acquire lock with retry for engine %(engId)s'),
              {'engId': engine_id}
              )
    return False


def sync_lock_release(context, engine_id):
    """Release the lock for the projects"""

    LOG.info('Releasing acquired lock with engine Id: %s' % engine_id)
    return db_api.sync_lock_release(context)
