...
class NeutronClient(base.DriverBase):
    ...
    def get_resource_usages(self, disabled_quotas, project_id):
        ...
        self.no_network = True if 'floatingip' in disabled_quotas else \
            False
        if not self.no_network:
            try:
                ...
                if "security_group" not in self.disabled_quotas or \
                    "security_group_rule" not in self.disabled_quotas:
