...
class KingbirdTestJSON(base.BaseKingbirdTest):
    ...
    def test_kingbird_delete_method(self):
        ...
        self.create_custom_kingbird_quota(FAKE_PROJECT,
                                          new_quota)
        deleted_quotas = self.delete_custom_kingbird_quota(FAKE_PROJECT,
                                                           quota_to_delete)
        expected_quotas = {u'Deleted quota limits': [u'cores']}
        self.assertEqual(expected_quotas, eval(deleted_quotas))
    ...
    def test_kingbird_delete_all_method(self):
        ...
        self.create_custom_kingbird_quota(FAKE_PROJECT,
                                          new_quota)
        deleted_quotas = self.delete_custom_kingbird_quota(FAKE_PROJECT)
        expected_quotas = u'Deleted all quota limits for the given project'
        self.assertEqual(expected_quotas, eval(deleted_quotas))

    def test_kingbird_get_default_method(self):
