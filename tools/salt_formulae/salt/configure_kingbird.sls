kingbird-conf:
  ...
  ini:
    ...
    - sections:
        ...
        database:
            {% if grains.get('database_type') == 'mysql' %}
                connection: mysql+pymysql://root:{{grains.get('database_password')}}@127.0.0.1/kingbird
            {% elif grains.get('database_type') == 'sqlite' %}
                connection: sqlite:///{{grains.get('service_name')}}.db
            {% else%}
                connection: ''
            {% endif%}
        keystone_authtoken:
            ...
            auth_url: {{pillar.get('auth_token_auth_url')}}
