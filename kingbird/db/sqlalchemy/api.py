...
def sync_lock_acquire(context, engine_id):
    lock = model_query(context, models.SyncLock).all()
    if not lock:
...
def sync_lock_release(context):
    session = _session(context)
    locks = model_query(context, models.SyncLock).all()
    for lock in locks:
        lock.delete(session=session)


def sync_lock_steal(context, engine_id):
    sync_lock_release(context)
    return sync_lock_acquire(context, engine_id)
