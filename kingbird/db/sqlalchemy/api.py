...
@require_context
def sync_job_create(context, job_id, status):
    with write_session() as session:
        time_now = timeutils.utcnow()
        sjc = models.SyncJob()
        sjc.id = job_id
        sjc.sync_status = status
...
def sync_job_delete(context, job_id):
    with write_session() as session:
        jobs = model_query(context, models.SyncJob). \
...
def resource_sync_update(context, job_id, region, resource, msg):
    with write_session() as session:
        time_now = timeutils.utcnow()
        resource_sync_ref = session.query(models.Resource_Sync).\
...
def resource_sync_list_by_job(context, job_id):
    rows = model_query(context, models.Resource_Sync).\
...
@require_context
def resource_sync_delete_by_job(context, job_id):
    with write_session() as session:
        jobs = model_query(context, models.Resource_Sync). \
