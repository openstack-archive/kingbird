...
class TestResourceManager(testroot.KBApiTest):
    ...
    @mock.patch.object(rpc_client, 'EngineClient')
    @mock.patch.object(sync_manager, 'db_api')
    def test_get_jobs_all_tenants(self, mock_db_api, mock_rpc_client):
        get_url = FAKE_URL + '/all'
        self.app.get(get_url, headers=FAKE_HEADERS)
        self.assertEqual(1, mock_db_api.sync_all_job_list.call_count)

    @mock.patch.object(rpc_client, 'EngineClient')
    @mock.patch.object(sync_manager, 'db_api')
    def test_get_jobs_non_admin(self, mock_db_api, mock_rpc_client):
        get_url = FAKE_URL + '/all'
        self.assertRaisesRegex(webtest.app.AppError, "401 *",
                               self.app.get, get_url,
