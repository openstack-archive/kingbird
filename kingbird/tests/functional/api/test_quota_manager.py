...
class TestQuotaManager(KBFunctionalTest):
    ...
    def test_get_all(self, mock_db_api):
        ...
        response = self.app.get(
            '/v1.0/fake_tenant_id/quota/tenant_1')
    ...
    def test_get(self, mock_db_api, mock_client):
        ...
        response = self.app.get(
            '/v1.0/fake_tenant_id/quota/tenant_1/ram')
        self.assertEqual(response.status_int, 200)
        self.assertEqual({"project_id": "tenant_1", "ram": 100},
                         eval(response.text))
    ...
    def test_post(self, mock_db_api, mock_client):
        ...
        response = self.app.post_json('/v1.0/fake_tenant_id/quota/tenant_1',
    ...
    def test_put(self, mock_db_api, mock_client):
        ...
        data = {"quota_set": {Res.resource: Res.hard_limit}}
        response = self.app.put_json('/v1.0/fake_tenant_id/quota/tenant_1',
                                     headers={'X-Tenant-Id': 'tenid'},
                                     params=data)
        self.assertEqual(response.status_int, 200)
    ...
    def test_delete(self, mock_db_api, mock_client):
        ...
        response = self.app.delete_json('/v1.0/fake_tenant_id/quota/tenant_1',
                                        headers={'X-Tenant-Id': 'tenid'},
                                        params=data)
        self.assertEqual(response.status_int, 200)
    ...
    def test_quota_sync(self, mock_client):
        response = self.app.post_json('/v1.0/quota/sync/tenant_1',
                                      headers={'X-Tenant-Id': 'tenid'})
        self.assertEqual(response.status_int, 200)
