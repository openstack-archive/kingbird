...
class SyncManager(manager.Manager):
    ...
    def keypair_sync_for_user(self, user_id, job_id, payload):
        ...
        resource_sync_details = db_api.\
            resource_sync_list_by_job(self.context, job_id)
        resource_sync_status = list()
        for sync_status in resource_sync_details:
            resource_sync_status.append(sync_status.get('sync_status'))
        result = "SUCCESS"
        if "FAILURE" in resource_sync_status:
            result = "FAILURE"
