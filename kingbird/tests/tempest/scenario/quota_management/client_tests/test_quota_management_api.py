...
class KingbirdQMTestJSON(base.BaseKingbirdTest):
    ...
    def test_kingbird_delete_all_method(self):
        ...
        self.delete_custom_kingbird_quota(
            self.resource_ids["target_project_id"])
        actual_quota_after_delete = self.get_custom_kingbird_quota(
            self.resource_ids["target_project_id"])
    ...
    def test_kingbird_create_quota_wrong_token(self):
        new_quota = {"instances": 15, "cores": 10}
        self.assertRaises(kingbirdclient.exceptions.APIException,
                          self.kingbird_create_quota_wrong_token,
                          self.resource_ids["project_id"], new_quota)

    def test_kingbird_get_method_wrong_token(self):
        self.assertRaises(kingbirdclient.exceptions.APIException,
                          self.kingbird_get_method_wrong_token,
                          self.resource_ids["target_project_id"])

    def test_kingbird_delete_method_wrong_token(self):
        self.assertRaises(kingbirdclient.exceptions.APIException,
                          self.kingbird_delete_method_wrong_token,
                          self.resource_ids["target_project_id"])
