...
if [[ "$Q_ENABLE_KINGBIRD" == "True" ]]; then
    if [[ "$1" == "stack" && "$2" == "pre-install" ]]; then
        ...
        if is_service_enabled kb-svc-jw; then
            run_process kb-svc-jw "python $KINGBIRD_JW_SERVICE --config-file=$KINGBIRD_JW_CONF"
        fi

        if is_service_enabled kb-svc-jd; then
            run_process kb-svc-jd "python $KINGBIRD_JD_SERVICE --config-file=$KINGBIRD_JD_CONF"
        fi
