...
class TestQuotaManager(base.KingbirdTestCase):
    ...
    @mock.patch.object(quota_manager.QuotaManager,
                       'get_tenant_quota_usage_per_region')
    def test_get_total_usage_for_tenant_read_error(self, mock_quota_usage,
                                                   mock_endpoint):
        qm = quota_manager.QuotaManager()
        mock_quota_usage.return_value = {}
        expected_output = qm.get_total_usage_for_tenant(FAKE_PROJECT)
        self.assertEqual(expected_output,
                         "Not able to read total usage for the Tenant")
