...
kingbird-engine talk with each other through message bus, and both
service will access to the database. Kingbird-api receives the RESTful
API request for quota management, and forward the request to kingbird-engine
to do quota synchronization etc task.

Therefore install Kingbird to the controller nodes of one of the OpenStack
...
In the folder of Kingbird, where you can find the source code of Kingbird,
...
prepare the folder used for cache/log/configuration for Kingbird:
...
Before edit the configuration file, prepare the database info for Kingbird.
...
For example, the following command will create database "kingbird", and grant
the previllesges for the db user "kingbird" with password "password":
...
To make the Kingbird can work normally, you have to edit these configuration
items. The [cache] section is used by kingbird engine to access the quota
information of Nova, Cinder, Neutron in each region, replace the auth_uri to
the keystone service in your environment, especially if the keystone service
is not located in the same node, and also for the account to access the Nova,
Cinder, Neutron in each region, in the following configuration, user "admin"
with password "password" of the tenant "admin" is configured to access
Nova, Cinder, Neutron in each region, the edpoints retrieved from KeyStone
will be cached in kingbird-engine to reduce the traffic to KeyStone:
...
on how to configure the keystone_authtoken secion for the keystonemiddleware
in detail, and change the following configuration accordingly based on your
environment:

*please specify the region_name where you want the token will be validated if
the KeyStone is deployed in multiple regions*
...
.. code-block:: bash
    ...
    openstack service create --name=kingbird kingbird
    openstack endpoint create --region=RegionOne --publicurl=http://127.0.0.1:8118/v1.0 --adminurl=http://127.0.0.1:8118/v1.0 --internalurl=http://127.0.0.1:8118/v1.0 kingbird
...
.. code-block:: bash
    ...
    curl  -H "Content-Type: application/json"  -H "X-Auth-Token: $token"  http://127.0.0.1:8118/
...
.. code-block:: bash
    ...
    curl  -H "Content-Type: application/json"  -H "X-Auth-Token: $token"  -H  "X_ROLE: admin"  -X PUT   http://127.0.0.1:8118/v1.0/$admin_project_id/os-quota-sets/$admin_project_id/sync
