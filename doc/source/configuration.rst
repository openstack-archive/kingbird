A brief introduction to configure Kingbird service. Only the
...
information of Nova, Cinder, Neutron in each region, in order to reduce
the load to KeyStone to retrieve the endpoint information in each time.
...
The [scheduler] section is used by kingbird engine to periodicly synchronize
...
- description: Boolean value for enable/disenable periodic tasks.
...
The [locks] section is used by kingbird engine to periodicly synchronize
and rebalance the quota for each project.
