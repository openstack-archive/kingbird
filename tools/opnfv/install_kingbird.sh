...
if [ $? -eq 0 ]; then
	...
	openstack endpoint create kingbird \
		--publicurl http://172.16.0.6:8118/v1 \
		--adminurl http://192.168.0.8:8118/v1 \
		--internalurl http://192.168.0.8:8118/v1 \
		--region RegionOne
fi
...
iniset $KINGBIRD_CONF_FILE cache identity_url $auth_uri
...
iniset $KINGBIRD_CONF_FILE oslo_messaging_rabbit rabbit_ha_queues True
...
nohup kingbird-engine --config-file $KINGBIRD_CONF_FILE > /var/log/kingbird-engine.log &
nohup kingbird-api --config-file $KINGBIRD_CONF_FILE > /var/log/kingbird-api &
