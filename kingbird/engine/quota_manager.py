...
class QuotaManager(manager.Manager):
    ...
    def periodic_balance_all(self):
        ...
        # using threads
        for current_project in sdk.OpenStackDriver().get_enabled_projects():
            thread = threading.Thread(target=self.quota_sync_for_project,
                                      args=(current_project,))
            projects_thread_list.append(thread)
            thread.start()
        # Wait for all the threads to complete the job(sync all projects quota)
    ...
    def quota_sync_for_project(self, project_id):
        ...
        regions_thread_list = []
        region_lists = self.endpoints.get_all_regions()
    ...
    def get_total_usage_for_tenant(self, project_id):
        ...
        return dict(self.get_summation(
